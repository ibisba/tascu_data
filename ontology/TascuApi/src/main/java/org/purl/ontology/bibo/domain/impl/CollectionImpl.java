...
import org.purl.ontology.bibo.domain.Collection;
 ...
 */
public class CollectionImpl extends LiteratureImpl implements Collection {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Collection";

  protected CollectionImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Collection make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new CollectionImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Collection.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Collection.class,false);
          if(toRet == null) {
            toRet = new CollectionImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Collection)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.CollectionImpl expected");
        }
      }
      return (Collection)toRet;
    }
  }
  ...
  public void remHasPart(Literature val) {
    this.remRef("http://purl.org/ontology/bibo/hasPart",val,true);
  }

  public List<? extends Literature> getAllHasPart() {
    return this.getRefSet("http://purl.org/ontology/bibo/hasPart",true,Literature.class);
  }

  public void addHasPart(Literature val) {
