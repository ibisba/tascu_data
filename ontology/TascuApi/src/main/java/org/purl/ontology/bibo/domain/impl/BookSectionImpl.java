...
import org.purl.ontology.bibo.domain.BookSection;
 ...
 */
public class BookSectionImpl extends DocumentPartImpl implements BookSection {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/BookSection";

  protected BookSectionImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static BookSection make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new BookSectionImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,BookSection.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,BookSection.class,false);
          if(toRet == null) {
            toRet = new BookSectionImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof BookSection)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.BookSectionImpl expected");
        }
      }
      return (BookSection)toRet;
    }
  }
  ...
  public String getChapter() {
    return this.getStringLit("http://purl.org/ontology/bibo/chapter",true);
  }

  public void setChapter(String val) {
    this.setStringLit("http://purl.org/ontology/bibo/chapter",val);
  }
