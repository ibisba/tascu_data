...
import org.purl.ontology.bibo.domain.Report;

/**
 * Code generated from http://purl.org/ontology/bibo/ ontology
 */
public class ReportImpl extends DocumentImpl implements Report {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Report";

  protected ReportImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Report make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new ReportImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Report.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Report.class,false);
          if(toRet == null) {
            toRet = new ReportImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Report)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.ReportImpl expected");
        }
      }
      return (Report)toRet;
    }
