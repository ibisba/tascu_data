...
import org.purl.ontology.bibo.domain.Manual;

/**
 * Code generated from http://purl.org/ontology/bibo/ ontology
 */
public class ManualImpl extends DocumentImpl implements Manual {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Manual";

  protected ManualImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Manual make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new ManualImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Manual.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Manual.class,false);
          if(toRet == null) {
            toRet = new ManualImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Manual)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.ManualImpl expected");
        }
      }
      return (Manual)toRet;
    }
