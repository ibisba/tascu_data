...
import org.purl.ontology.bibo.domain.Statute;

/**
 * Code generated from http://purl.org/ontology/bibo/ ontology
 */
public class StatuteImpl extends LegislationImpl implements Statute {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Statute";

  protected StatuteImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Statute make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new StatuteImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Statute.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Statute.class,false);
          if(toRet == null) {
            toRet = new StatuteImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Statute)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.StatuteImpl expected");
        }
      }
      return (Statute)toRet;
    }
  }
