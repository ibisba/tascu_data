...
import org.purl.ontology.bibo.domain.CollectedDocument;
 ...
 */
public class CollectedDocumentImpl extends DocumentImpl implements CollectedDocument {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/CollectedDocument";

  protected CollectedDocumentImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static CollectedDocument make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new CollectedDocumentImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,CollectedDocument.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,CollectedDocument.class,false);
          if(toRet == null) {
            toRet = new CollectedDocumentImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof CollectedDocument)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.CollectedDocumentImpl expected");
        }
      }
      return (CollectedDocument)toRet;
    }
  }
  ...
  public List<? extends Document> getAllHasPart() {
    return this.getRefSet("http://purl.org/ontology/bibo/hasPart",false,Document.class);
  }
