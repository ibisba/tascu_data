...
import org.purl.ontology.bibo.domain.Series;

/**
 * Code generated from http://purl.org/ontology/bibo/ ontology
 */
public class SeriesImpl extends CollectionImpl implements Series {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Series";

  protected SeriesImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Series make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new SeriesImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Series.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Series.class,false);
          if(toRet == null) {
            toRet = new SeriesImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Series)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.SeriesImpl expected");
        }
      }
      return (Series)toRet;
    }
  ...
  public void remHasPart(Document val) {
    this.remRef("http://purl.org/ontology/bibo/hasPart",val,true);
  }

  public List<? extends Document> getAllHasPart() {
    return this.getRefSet("http://purl.org/ontology/bibo/hasPart",true,Document.class);
  }

  public void addHasPart(Document val) {
