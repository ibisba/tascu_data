 ...
 */
public class SlideImpl extends DocumentPartImpl implements Slide {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Slide";

  protected SlideImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Slide make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new SlideImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Slide.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Slide.class,false);
          if(toRet == null) {
            toRet = new SlideImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Slide)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.SlideImpl expected");
        }
      }
      return (Slide)toRet;
    }
