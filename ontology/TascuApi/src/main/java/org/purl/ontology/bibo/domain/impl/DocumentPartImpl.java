...
import org.purl.ontology.bibo.domain.DocumentPart;
 ...
 */
public class DocumentPartImpl extends DocumentImpl implements DocumentPart {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/DocumentPart";

  protected DocumentPartImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static DocumentPart make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new DocumentPartImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,DocumentPart.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,DocumentPart.class,false);
          if(toRet == null) {
            toRet = new DocumentPartImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof DocumentPart)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.DocumentPartImpl expected");
        }
      }
      return (DocumentPart)toRet;
    }
  }
