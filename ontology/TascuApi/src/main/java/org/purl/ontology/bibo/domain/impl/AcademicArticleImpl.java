 ...
 */
public class AcademicArticleImpl extends ArticleImpl implements AcademicArticle {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/AcademicArticle";

  protected AcademicArticleImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static AcademicArticle make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new AcademicArticleImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,AcademicArticle.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,AcademicArticle.class,false);
          if(toRet == null) {
            toRet = new AcademicArticleImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof AcademicArticle)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.AcademicArticleImpl expected");
        }
      }
      return (AcademicArticle)toRet;
    }
  }
