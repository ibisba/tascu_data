...
import org.purl.ontology.bibo.domain.Article;
 ...
 */
public class IssueImpl extends CollectedDocumentImpl implements Issue {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Issue";

  protected IssueImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Issue make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new IssueImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Issue.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Issue.class,false);
          if(toRet == null) {
            toRet = new IssueImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Issue)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.IssueImpl expected");
        }
      }
      return (Issue)toRet;
    }
  ...
  public void validate() {
    ...
    this.checkCardMin1("http://purl.org/ontology/bibo/issue");
  }

  public void remHasPart(Article val) {
    this.remRef("http://purl.org/ontology/bibo/hasPart",val,false);
  }

  public List<? extends Article> getAllHasPart() {
    return this.getRefSet("http://purl.org/ontology/bibo/hasPart",false,Article.class);
  }

  public void addHasPart(Article val) {
    this.addRef("http://purl.org/ontology/bibo/hasPart",val);
  }

  public String getIssue() {
    return this.getStringLit("http://purl.org/ontology/bibo/issue",false);
  }

  public void setIssue(String val) {
    this.setStringLit("http://purl.org/ontology/bibo/issue",val);
  }
