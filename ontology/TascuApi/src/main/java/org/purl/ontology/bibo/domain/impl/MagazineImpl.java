...
import org.purl.ontology.bibo.domain.Magazine;

/**
 * Code generated from http://purl.org/ontology/bibo/ ontology
 */
public class MagazineImpl extends PeriodicalImpl implements Magazine {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Magazine";

  protected MagazineImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Magazine make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new MagazineImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Magazine.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Magazine.class,false);
          if(toRet == null) {
            toRet = new MagazineImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Magazine)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.MagazineImpl expected");
        }
      }
      return (Magazine)toRet;
    }
  }
