...
import org.purl.ontology.bibo.domain.Proceedings;

/**
 * Code generated from http://purl.org/ontology/bibo/ ontology
 */
public class ProceedingsImpl extends BookImpl implements Proceedings {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Proceedings";

  protected ProceedingsImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Proceedings make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new ProceedingsImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Proceedings.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Proceedings.class,false);
          if(toRet == null) {
            toRet = new ProceedingsImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Proceedings)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.ProceedingsImpl expected");
        }
      }
      return (Proceedings)toRet;
    }
  }
