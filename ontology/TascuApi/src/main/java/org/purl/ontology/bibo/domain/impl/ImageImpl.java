...
import org.purl.ontology.bibo.domain.Image;
import org.purl.ontology.bibo.domain.Literature;

/**
 * Code generated from http://purl.org/ontology/bibo/ ontology
 */
public class ImageImpl extends DocumentImpl implements Image {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Image";

  protected ImageImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Image make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new ImageImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Image.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Image.class,false);
          if(toRet == null) {
            toRet = new ImageImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Image)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.ImageImpl expected");
        }
      }
      return (Image)toRet;
    }
