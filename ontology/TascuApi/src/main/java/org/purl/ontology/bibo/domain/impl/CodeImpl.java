...
import org.purl.ontology.bibo.domain.Code;
import org.purl.ontology.bibo.domain.Document;
import org.purl.ontology.bibo.domain.DocumentStatus;
import org.purl.ontology.bibo.domain.Legislation;
import org.purl.ontology.bibo.domain.Literature;

/**
 * Code generated from http://purl.org/ontology/bibo/ ontology
 */
public class CodeImpl extends PeriodicalImpl implements Code {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Code";

  protected CodeImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Code make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new CodeImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Code.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Code.class,false);
          if(toRet == null) {
            toRet = new CodeImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Code)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.CodeImpl expected");
        }
      }
      return (Code)toRet;
    }
  ...
  public void remHasPart(Legislation val) {
    this.remRef("http://purl.org/ontology/bibo/hasPart",val,false);
  }

  public List<? extends Legislation> getAllHasPart() {
    return this.getRefSet("http://purl.org/ontology/bibo/hasPart",false,Legislation.class);
  }

  public void addHasPart(Legislation val) {
