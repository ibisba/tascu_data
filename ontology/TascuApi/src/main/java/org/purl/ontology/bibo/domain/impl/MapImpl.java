...
import org.purl.ontology.bibo.domain.Map;

/**
 * Code generated from http://purl.org/ontology/bibo/ ontology
 */
public class MapImpl extends ImageImpl implements Map {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Map";

  protected MapImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Map make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new MapImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Map.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Map.class,false);
          if(toRet == null) {
            toRet = new MapImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Map)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.MapImpl expected");
        }
      }
      return (Map)toRet;
    }
