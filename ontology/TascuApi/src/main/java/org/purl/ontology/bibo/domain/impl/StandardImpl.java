...
import org.purl.ontology.bibo.domain.Standard;
...
public class StandardImpl extends DocumentImpl implements Standard {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Standard";

  protected StandardImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Standard make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new StandardImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Standard.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Standard.class,false);
          if(toRet == null) {
            toRet = new StandardImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Standard)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.StandardImpl expected");
        }
      }
      return (Standard)toRet;
    }
  }
