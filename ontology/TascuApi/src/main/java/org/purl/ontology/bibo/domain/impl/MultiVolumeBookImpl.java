...
import org.purl.ontology.bibo.domain.MultiVolumeBook;

/**
 * Code generated from http://purl.org/ontology/bibo/ ontology
 */
public class MultiVolumeBookImpl extends CollectionImpl implements MultiVolumeBook {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/MultiVolumeBook";

  protected MultiVolumeBookImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static MultiVolumeBook make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new MultiVolumeBookImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,MultiVolumeBook.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,MultiVolumeBook.class,false);
          if(toRet == null) {
            toRet = new MultiVolumeBookImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof MultiVolumeBook)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.MultiVolumeBookImpl expected");
        }
      }
      return (MultiVolumeBook)toRet;
    }
  }
  ...
  public void remHasPart(Book val) {
    this.remRef("http://purl.org/ontology/bibo/hasPart",val,true);
  }

  public List<? extends Book> getAllHasPart() {
    return this.getRefSet("http://purl.org/ontology/bibo/hasPart",true,Book.class);
  }

  public void addHasPart(Book val) {
