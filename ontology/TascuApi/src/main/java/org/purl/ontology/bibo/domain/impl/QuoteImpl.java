...
import org.purl.ontology.bibo.domain.Quote;

/**
 * Code generated from http://purl.org/ontology/bibo/ ontology
 */
public class QuoteImpl extends ExcerptImpl implements Quote {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Quote";

  protected QuoteImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Quote make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new QuoteImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Quote.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Quote.class,false);
          if(toRet == null) {
            toRet = new QuoteImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Quote)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.QuoteImpl expected");
        }
      }
      return (Quote)toRet;
    }
