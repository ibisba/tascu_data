...
import org.purl.ontology.bibo.domain.AudioDocument;
...
public class AudioDocumentImpl extends DocumentImpl implements AudioDocument {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/AudioDocument";

  protected AudioDocumentImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static AudioDocument make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new AudioDocumentImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,AudioDocument.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,AudioDocument.class,false);
          if(toRet == null) {
            toRet = new AudioDocumentImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof AudioDocument)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.AudioDocumentImpl expected");
        }
      }
      return (AudioDocument)toRet;
    }
  }
