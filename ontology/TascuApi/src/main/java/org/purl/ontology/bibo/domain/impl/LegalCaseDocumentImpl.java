...
import org.purl.ontology.bibo.domain.LegalCaseDocument;
import org.purl.ontology.bibo.domain.Literature;

/**
 * Code generated from http://purl.org/ontology/bibo/ ontology
 */
public class LegalCaseDocumentImpl extends LegalDocumentImpl implements LegalCaseDocument {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/LegalCaseDocument";

  protected LegalCaseDocumentImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static LegalCaseDocument make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new LegalCaseDocumentImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,LegalCaseDocument.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,LegalCaseDocument.class,false);
          if(toRet == null) {
            toRet = new LegalCaseDocumentImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof LegalCaseDocument)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.LegalCaseDocumentImpl expected");
        }
      }
      return (LegalCaseDocument)toRet;
    }
  }
