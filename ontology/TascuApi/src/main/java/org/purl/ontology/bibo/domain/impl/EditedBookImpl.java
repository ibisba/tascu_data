...
import org.purl.ontology.bibo.domain.EditedBook;
import org.purl.ontology.bibo.domain.Literature;

/**
 * Code generated from http://purl.org/ontology/bibo/ ontology
 */
public class EditedBookImpl extends CollectedDocumentImpl implements EditedBook {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/EditedBook";

  protected EditedBookImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static EditedBook make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new EditedBookImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,EditedBook.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,EditedBook.class,false);
          if(toRet == null) {
            toRet = new EditedBookImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof EditedBook)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.EditedBookImpl expected");
        }
      }
      return (EditedBook)toRet;
    }
  }
