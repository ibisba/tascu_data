...
import org.purl.ontology.bibo.domain.Film;
import org.purl.ontology.bibo.domain.Literature;

/**
 * Code generated from http://purl.org/ontology/bibo/ ontology
 */
public class FilmImpl extends AudioVisualDocumentImpl implements Film {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Film";

  protected FilmImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Film make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new FilmImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Film.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Film.class,false);
          if(toRet == null) {
            toRet = new FilmImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Film)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.FilmImpl expected");
        }
      }
      return (Film)toRet;
    }
