...
import org.purl.ontology.bibo.domain.Chapter;
 ...
 */
public class ChapterImpl extends BookSectionImpl implements Chapter {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Chapter";

  protected ChapterImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Chapter make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new ChapterImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Chapter.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Chapter.class,false);
          if(toRet == null) {
            toRet = new ChapterImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Chapter)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.ChapterImpl expected");
        }
      }
      return (Chapter)toRet;
    }
  }
