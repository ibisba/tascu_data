...
import org.purl.ontology.bibo.domain.Bill;
 ...
 */
public class BillImpl extends LegislationImpl implements Bill {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Bill";

  protected BillImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Bill make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new BillImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Bill.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Bill.class,false);
          if(toRet == null) {
            toRet = new BillImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Bill)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.BillImpl expected");
        }
      }
      return (Bill)toRet;
    }
