 ...
 */
public class LegislationImpl extends LegalDocumentImpl implements Legislation {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Legislation";

  protected LegislationImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Legislation make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new LegislationImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Legislation.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Legislation.class,false);
          if(toRet == null) {
            toRet = new LegislationImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Legislation)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.LegislationImpl expected");
        }
      }
      return (Legislation)toRet;
    }
  }
