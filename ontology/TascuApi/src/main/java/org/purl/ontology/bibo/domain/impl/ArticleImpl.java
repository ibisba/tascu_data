...
public class ArticleImpl extends DocumentImpl implements Article {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Article";

  protected ArticleImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Article make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new ArticleImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Article.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Article.class,false);
          if(toRet == null) {
            toRet = new ArticleImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Article)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.ArticleImpl expected");
        }
      }
