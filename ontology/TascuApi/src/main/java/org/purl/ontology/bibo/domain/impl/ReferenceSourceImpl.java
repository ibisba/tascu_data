...
import org.purl.ontology.bibo.domain.ReferenceSource;

/**
 * Code generated from http://purl.org/ontology/bibo/ ontology
 */
public class ReferenceSourceImpl extends DocumentImpl implements ReferenceSource {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/ReferenceSource";

  protected ReferenceSourceImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static ReferenceSource make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new ReferenceSourceImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,ReferenceSource.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,ReferenceSource.class,false);
          if(toRet == null) {
            toRet = new ReferenceSourceImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof ReferenceSource)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.ReferenceSourceImpl expected");
        }
      }
      return (ReferenceSource)toRet;
    }
  }
