...
import org.purl.ontology.bibo.domain.Excerpt;
import org.purl.ontology.bibo.domain.Literature;

/**
 * Code generated from http://purl.org/ontology/bibo/ ontology
 */
public class ExcerptImpl extends DocumentPartImpl implements Excerpt {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Excerpt";

  protected ExcerptImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Excerpt make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new ExcerptImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Excerpt.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Excerpt.class,false);
          if(toRet == null) {
            toRet = new ExcerptImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Excerpt)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.ExcerptImpl expected");
        }
      }
      return (Excerpt)toRet;
    }
  }
