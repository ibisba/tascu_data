 ...
 */
public class BookImpl extends DocumentImpl implements Book {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Book";

  protected BookImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Book make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new BookImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Book.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Book.class,false);
          if(toRet == null) {
            toRet = new BookImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Book)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.BookImpl expected");
        }
      }
      return (Book)toRet;
    }
