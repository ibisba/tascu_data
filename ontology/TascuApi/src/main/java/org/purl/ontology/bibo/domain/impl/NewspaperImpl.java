...
import org.purl.ontology.bibo.domain.Newspaper;

/**
 * Code generated from http://purl.org/ontology/bibo/ ontology
 */
public class NewspaperImpl extends PeriodicalImpl implements Newspaper {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Newspaper";

  protected NewspaperImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Newspaper make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new NewspaperImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Newspaper.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Newspaper.class,false);
          if(toRet == null) {
            toRet = new NewspaperImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Newspaper)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.NewspaperImpl expected");
        }
      }
      return (Newspaper)toRet;
    }
  }
  ...
  public void remHasPart(Issue val) {
    this.remRef("http://purl.org/ontology/bibo/hasPart",val,false);
  }

  public List<? extends Issue> getAllHasPart() {
    return this.getRefSet("http://purl.org/ontology/bibo/hasPart",false,Issue.class);
  }

  public void addHasPart(Issue val) {
