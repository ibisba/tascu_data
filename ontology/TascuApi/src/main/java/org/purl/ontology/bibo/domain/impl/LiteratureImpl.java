 ...
 */
public class LiteratureImpl extends OWLThingImpl implements Literature {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Literature";

  protected LiteratureImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Literature make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new LiteratureImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Literature.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Literature.class,false);
          if(toRet == null) {
            toRet = new LiteratureImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Literature)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.LiteratureImpl expected");
        }
      }
      return (Literature)toRet;
    }
  }
