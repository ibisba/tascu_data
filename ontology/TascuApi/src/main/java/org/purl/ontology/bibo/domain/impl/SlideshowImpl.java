...
import org.purl.ontology.bibo.domain.Slide;
import org.purl.ontology.bibo.domain.Slideshow;

/**
 * Code generated from http://purl.org/ontology/bibo/ ontology
 */
public class SlideshowImpl extends DocumentImpl implements Slideshow {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Slideshow";

  protected SlideshowImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Slideshow make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new SlideshowImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Slideshow.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Slideshow.class,false);
          if(toRet == null) {
            toRet = new SlideshowImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Slideshow)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.SlideshowImpl expected");
        }
      }
      return (Slideshow)toRet;
    }
  }
  ...
  public void remHasPart(Slide val) {
    this.remRef("http://purl.org/ontology/bibo/hasPart",val,true);
  }

  public List<? extends Slide> getAllHasPart() {
    return this.getRefSet("http://purl.org/ontology/bibo/hasPart",true,Slide.class);
  }

  public void addHasPart(Slide val) {
