...
import org.purl.ontology.bibo.domain.Email;
import org.purl.ontology.bibo.domain.Literature;

/**
 * Code generated from http://purl.org/ontology/bibo/ ontology
 */
public class EmailImpl extends PersonalCommunicationDocumentImpl implements Email {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Email";

  protected EmailImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Email make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new EmailImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Email.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Email.class,false);
          if(toRet == null) {
            toRet = new EmailImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Email)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.EmailImpl expected");
        }
      }
      return (Email)toRet;
    }
