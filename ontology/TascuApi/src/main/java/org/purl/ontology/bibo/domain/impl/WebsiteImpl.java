...
import org.purl.ontology.bibo.domain.Website;

/**
 * Code generated from http://purl.org/ontology/bibo/ ontology
 */
public class WebsiteImpl extends CollectionImpl implements Website {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Website";

  protected WebsiteImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Website make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new WebsiteImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Website.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Website.class,false);
          if(toRet == null) {
            toRet = new WebsiteImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Website)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.WebsiteImpl expected");
        }
      }
      return (Website)toRet;
    }
  }
  ...
  public void remHasPart(Webpage val) {
    this.remRef("http://purl.org/ontology/bibo/hasPart",val,false);
  }

  public List<? extends Webpage> getAllHasPart() {
    return this.getRefSet("http://purl.org/ontology/bibo/hasPart",false,Webpage.class);
  }

  public void addHasPart(Webpage val) {
