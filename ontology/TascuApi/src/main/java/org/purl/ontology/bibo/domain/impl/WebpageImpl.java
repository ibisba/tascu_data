...
public class WebpageImpl extends DocumentImpl implements Webpage {
  ...
  public static Webpage make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new WebpageImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Webpage.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Webpage.class,false);
          if(toRet == null) {
            toRet = new WebpageImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Webpage)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.WebpageImpl expected");
        }
      }
      return (Webpage)toRet;
    }
  }
