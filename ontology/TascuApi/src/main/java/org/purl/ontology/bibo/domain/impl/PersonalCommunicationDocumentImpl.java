...
import org.purl.ontology.bibo.domain.PersonalCommunicationDocument;

/**
 * Code generated from http://purl.org/ontology/bibo/ ontology
 */
public class PersonalCommunicationDocumentImpl extends DocumentImpl implements PersonalCommunicationDocument {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/PersonalCommunicationDocument";

  protected PersonalCommunicationDocumentImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static PersonalCommunicationDocument make(Domain domain, Resource resource,
      boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new PersonalCommunicationDocumentImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,PersonalCommunicationDocument.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,PersonalCommunicationDocument.class,false);
          if(toRet == null) {
            toRet = new PersonalCommunicationDocumentImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof PersonalCommunicationDocument)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.PersonalCommunicationDocumentImpl expected");
        }
      }
      return (PersonalCommunicationDocument)toRet;
    }
  }
