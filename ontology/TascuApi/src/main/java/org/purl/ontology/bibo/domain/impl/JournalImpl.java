 ...
 */
public class JournalImpl extends PeriodicalImpl implements Journal {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Journal";

  protected JournalImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Journal make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new JournalImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Journal.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Journal.class,false);
          if(toRet == null) {
            toRet = new JournalImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Journal)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.JournalImpl expected");
        }
      }
      return (Journal)toRet;
    }
  }
