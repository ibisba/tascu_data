 ...
 */
public class DocumentImpl extends LiteratureImpl implements Document {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Document";

  protected DocumentImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Document make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new DocumentImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Document.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Document.class,false);
          if(toRet == null) {
            toRet = new DocumentImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Document)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.DocumentImpl expected");
        }
      }
      return (Document)toRet;
    }
  }
