...
import org.purl.ontology.bibo.domain.Periodical;

/**
 * Code generated from http://purl.org/ontology/bibo/ ontology
 */
public class PeriodicalImpl extends CollectionImpl implements Periodical {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Periodical";

  protected PeriodicalImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Periodical make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new PeriodicalImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Periodical.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Periodical.class,false);
          if(toRet == null) {
            toRet = new PeriodicalImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Periodical)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.PeriodicalImpl expected");
        }
      }
      return (Periodical)toRet;
    }
  }
