...
import org.purl.ontology.bibo.domain.Brief;
 ...
 */
public class BriefImpl extends LegalCaseDocumentImpl implements Brief {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Brief";

  protected BriefImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Brief make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new BriefImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Brief.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Brief.class,false);
          if(toRet == null) {
            toRet = new BriefImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Brief)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.BriefImpl expected");
        }
      }
      return (Brief)toRet;
    }
