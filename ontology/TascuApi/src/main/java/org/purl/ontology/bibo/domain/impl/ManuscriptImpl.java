 ...
 */
public class ManuscriptImpl extends DocumentImpl implements Manuscript {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Manuscript";

  protected ManuscriptImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Manuscript make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new ManuscriptImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Manuscript.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Manuscript.class,false);
          if(toRet == null) {
            toRet = new ManuscriptImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Manuscript)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.ManuscriptImpl expected");
        }
      }
      return (Manuscript)toRet;
    }
  }
