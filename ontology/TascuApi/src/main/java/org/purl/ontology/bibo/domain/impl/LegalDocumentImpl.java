 ...
 */
public class LegalDocumentImpl extends DocumentImpl implements LegalDocument {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/LegalDocument";

  protected LegalDocumentImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static LegalDocument make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new LegalDocumentImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,LegalDocument.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,LegalDocument.class,false);
          if(toRet == null) {
            toRet = new LegalDocumentImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof LegalDocument)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.LegalDocumentImpl expected");
        }
      }
      return (LegalDocument)toRet;
    }
  }

  public void validate() {
    super.validate();
    this.checkCardMin1("http://purl.org/ontology/bibo/court");
    this.checkCardMin1("http://purl.org/ontology/bibo/argued");
  }
