...
import org.purl.ontology.bibo.domain.AudioVisualDocument;
 ...
 */
public class AudioVisualDocumentImpl extends DocumentImpl implements AudioVisualDocument {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/AudioVisualDocument";

  protected AudioVisualDocumentImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static AudioVisualDocument make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new AudioVisualDocumentImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,AudioVisualDocument.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,AudioVisualDocument.class,false);
          if(toRet == null) {
            toRet = new AudioVisualDocumentImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof AudioVisualDocument)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.AudioVisualDocumentImpl expected");
        }
      }
      return (AudioVisualDocument)toRet;
    }
  }
  ...
  public void remDirector(Agent val) {
    this.remRef("http://purl.org/ontology/bibo/director",val,true);
  }

  public List<? extends Agent> getAllDirector() {
    return this.getRefSet("http://purl.org/ontology/bibo/director",true,Agent.class);
  }

  public void addDirector(Agent val) {
    this.addRef("http://purl.org/ontology/bibo/director",val);
  }
