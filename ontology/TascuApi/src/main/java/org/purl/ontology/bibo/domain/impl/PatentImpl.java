...
import org.purl.ontology.bibo.domain.Patent;

/**
 * Code generated from http://purl.org/ontology/bibo/ ontology
 */
public class PatentImpl extends DocumentImpl implements Patent {
  public static final String TypeIRI = "http://purl.org/ontology/bibo/Patent";

  protected PatentImpl(Domain domain, Resource resource) {
    super(domain,resource);
  }

  public static Patent make(Domain domain, Resource resource, boolean direct) {
    synchronized(domain) {
      Object toRet = null;
      if(direct) {
        toRet = new PatentImpl(domain,resource);;
      }
      else {
        toRet = domain.getObject(resource,Patent.class);
        if(toRet == null) {
          toRet = domain.getObjectFromResource(resource,Patent.class,false);
          if(toRet == null) {
            toRet = new PatentImpl(domain,resource);;
          }
        }
        else if(!(toRet instanceof Patent)) {
          throw new RuntimeException("Instance of org.purl.ontology.bibo.domain.impl.PatentImpl expected");
        }
      }
      return (Patent)toRet;
    }
